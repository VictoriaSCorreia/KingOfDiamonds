...
public class Game {
    ...
    public void login(){
        // creating the players and adding them to the players list
        for (int i = 0; i < 5; i++){
            Player player = new Player();
            player.enterName();
            round.playersList.add(player);
        }
        // creating a backup players list
        for(Player player : round.playersList)
            round.playersList2.add(player);
    }
    public void start(){     
        // it will run until there's a winner
        while (round.getRoundNum() > 1){
            double result;
            int winnerMove, numberOfWinners;
            ...
            numberOfWinners = numberOfWinners(winnerMove);
            
            rules.setResult(result);
            rules.setWinnerMove(winnerMove);
            rules.setnumberOfWinners(numberOfWinners);
    ...
    public int numberOfWinners(int winnerMove){
        int winners = 0;
        for (Player player : round.playersList){
